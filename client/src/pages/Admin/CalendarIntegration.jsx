...
import { Calendar, Trash, ArrowUpRight } from "@phosphor-icons/react";
...
export default function CalendarIntegration() {
    ...
    try {
      ...
      if (data?.calendar) {
        setIntegration(prev => ({
          ...
          ...data.calendar,
          loading: false,
          connected: !!(data.calendar.enabled && data.calendar.email),
        }));
      } else {
  ...
  async function handleConnectGoogle() {
    try {
      ...
      const authUrl = await dataService.getGoogleAuthUrl('calendar');
  ...
  async function handleTestConnection() {
    try {
      setIntegration(prev => ({ ...prev, saving: true, error: null }));
      const result = await dataService.testGoogleIntegration('calendar');
  ...
  async function handleDisconnect() {
    if (!window.confirm('Disconnect Google Calendar access?')) return;
    try {
      setIntegration(prev => ({ ...prev, saving: true, error: null }));
      await dataService.disconnectGoogleIntegration('calendar');
  ...
  if (integration.loading) {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-extrabold tracking-tight">Calendar Integration</h2>
      ...
      <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="font-extrabold text-lg flex items-center gap-2">
            <Calendar size={20} weight="fill" className="text-blue-400" />
