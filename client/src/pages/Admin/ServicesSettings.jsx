...
export default function ServicesSettings() {
  ...
  const fetchServices = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await dataService.getAdminServices();
  ...
  async function handleCreateItem() {
    try {
      ...
      await dataService.createService(newItem);
  ...
  async function handleUpdateItem(id, updatedItem) {
    try {
      setSaving(true);
      setError(null);
      await dataService.updateService(id, updatedItem);
      setSuccess(true);
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const icons = ['Code', 'Stack', 'CloudArrowUp', 'Cpu', 'DeviceMobile', 'Database', 'Drop', 'Sparkle', 'ShoppingBag', 'Rocket', 'Briefcase', 'Globe', 'Lock', 'Shield', 'Heart', 'Star'];

  if (loading) {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-extrabold tracking-tight">Services Settings</h2>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {success && (
          <div className="bg-emerald-950/30 border border-emerald-900/30 text-emerald-400 rounded-xl p-4 text-sm self-center">
            Services saved successfully with real-time sync!
          </div>
        )}
      </div>
        ...
        <div key={item._id || idx} className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 space-y-3 flex gap-4">
          <div className="flex-1 space-y-3">
            ...
            <select
              ...
              className="w-48 bg-slate-950 border border-slate-700/50 rounded-xl px-3 py-2 text-sm"
            >
              {icons.map(icon => (
