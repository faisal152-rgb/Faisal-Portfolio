...
import { WhatsappLogo, Trash, ArrowUpRight } from "@phosphor-icons/react";
...
export default function WhatsAppIntegration() {
    ...
    try {
      ...
      if (data?.whatsapp) {
        setIntegration(prev => ({
          ...
          ...data.whatsapp,
          loading: false,
          connected: !!(data.whatsapp.enabled && (data.whatsapp.phoneNumber || data.whatsapp.phoneNumberId)),
        }));
      } else {
  ...
  useEffect(() => {
    ...
    // Check for connection status after setup
    const params = new URLSearchParams(window.location.search);
    if (params.get('integration') === 'whatsapp' && params.get('status') === 'connected') {
  ...
  async function handleConnect() {
    try {
      ...
      const result = await dataService.connectWhatsAppIntegration();
      setIntegration(prev => ({ ...prev, ...result, saving: false, enabled: true, connected: true, success: true }));
  ...
  async function handleTestConnection() {
    try {
      setIntegration(prev => ({ ...prev, saving: true, error: null }));
      const result = await dataService.testWhatsAppIntegration();
      setIntegration(prev => ({ ...prev, saving: false, connected: true, success: true, phoneNumber: result.phoneNumber || prev.phoneNumber }));
      setTimeout(() => setIntegration(prev => ({ ...prev, success: false })), 3000);
    } catch (error) {
      setIntegration(prev => ({ ...prev, saving: false, error: error.message, connected: false }));
    }
  ...
  async function handleDisconnect() {
    if (!window.confirm('Disconnect WhatsApp access?')) return;
    try {
      setIntegration(prev => ({ ...prev, saving: true, error: null }));
      await dataService.disconnectWhatsAppIntegration();
      setIntegration(prev => ({ ...prev, saving: false, connected: false, enabled: false, phoneNumber: '', success: true }));
      setTimeout(() => setIntegration(prev => ({ ...prev, success: false })), 3000);
    } catch (error) {
  ...
  if (integration.loading) {
    return (
      <div className="space-y-6">
        <h2 className="text-2xl font-extrabold tracking-tight">WhatsApp Integration</h2>
      ...
      <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="font-extrabold text-lg flex items-center gap-2">
            <WhatsappLogo size={20} weight="fill" className="text-green-400" />
        ...
        {integration.phoneNumber && (
          <div className="text-sm text-slate-300">
            Connected number: <span className="font-semibold text-white">{integration.phoneNumber}</span>
          </div>
        )}
